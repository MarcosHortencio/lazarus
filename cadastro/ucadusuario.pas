...
uses
  ...
  StdCtrls, Buttons, DbCtrls, ComCtrls, MaskEdit, DBGrids, PopupNotifier,
  utelaheranca, db, cCadUsuario, uEnum, uConexao, uUtils;
...
type
  ...
  TfrmCadUsuario = class(TfrmTelaHeranca)
    ...
    PoupNotificacao: TPopupNotifier;
    TmrPoup: TTimer;
    ...
    procedure TmrPoupTimer(Sender: TObject);

  private
    procedure MensagemPoup(Mensagem:string);
  public
    oUsuario:TUsuario;
...
begin
  ...
  if oUsuario.UsuarioExiste(edtNome.Text, aCodigo) then begin
    //MessageOK('Usuário já cadastrado', mtInformation);
     MensagemPoup('Usuário: '+ edtNome.text +'  já cadastrado ! ');
    edtNome.SetFocus;
    abort;
  end;
  ...
  MensagemPoup('gravado com sucesso ! ');
  inherited;
end;
...
procedure TfrmCadUsuario.TmrPoupTimer(Sender: TObject);
begin
  PoupNotificacao.Hide;
  TmrPoup.Enabled:=false;
end;

procedure TfrmCadUsuario.MensagemPoup(Mensagem: string);
var direita,topo : integer;
begin
  PoupNotificacao.Title:='';
  PoupNotificacao.Text:=mensagem;
  //PoupNotificacao.ShowAtPos(GetSidePosition(akRight),Left);

  direita:=frmPrincipal.Width-Length(PoupNotificacao.Text);
  topo:=110;
  //ShowMessage(IntToStr(tmn));
   PoupNotificacao.ShowAtPos(direita,topo);
   TmrPoup.Enabled:=true;
end;
